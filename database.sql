...
create table
  public.calendar_events (
    ...
    type text check (
      type in ('career', 'academic', 'wellness')
    ),
...
create policy "Profiles are viewable by owner" on profiles for
select
  using (auth.uid () = id);
...
create policy "Users can insert their own calendar events" on calendar_events for insert
with
  check (auth.uid () = user_id);
...
drop trigger if exists handle_preferences_updated_at on user_preferences;

drop trigger if exists handle_calendar_events_updated_at on calendar_events;
...
create trigger handle_preferences_updated_at before
update on user_preferences for each row
execute function handle_updated_at ();

create trigger handle_calendar_events_updated_at before
